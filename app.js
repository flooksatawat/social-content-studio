...
function buildFacebookPost(brief, strategy, hooks, hashtags) {
  ...
  const audienceLines = [
    `คนที่กำลังเริ่มวางแผนคุ้มครอง`,
    `คนที่อยากดูแลครอบครัวแต่ยังไม่แน่ใจว่าจะเริ่มจากตรงไหน`,
    `คนที่มีภาระจริงและต้องการแผนที่จ่ายไหวต่อเนื่อง`,
  ];
  const opening = [
    `## ${hooks[0]} ##`,
    `.`,
    `ขอชวนมองเรื่อง ${keyword} แบบที่ใช้ได้จริงกับชีวิตก่อนนะครับ`,
    `ไม่ใช่เริ่มจาก “แผนไหนขายดี” แต่เริ่มจาก “ชีวิตจริงของเราเป็นแบบไหน”`,
    `.`,
    `คนที่เหมาะกับโพสต์นี้คือ`,
    ...audienceLines.map((line, index) => `${index + 1}. ${line}`),
    `.`,
    `และนี่คือจุดที่คนส่วนใหญ่พลาด`,
    `พยายามเลือกจากเบี้ยอย่างเดียว ทั้งที่สิ่งที่ควรรู้ก่อนคือใครต้องได้รับการดูแล ภาระอะไรต้องรับมือ และงบที่สบายใจกับชีวิตจริงอยู่ตรงไหน`,
    `.`,
    `### Key message ###`,
    `ประกันชีวิตที่ดี ไม่ใช่แผนที่ถูกที่สุด แต่คือแผนที่ช่วยให้ครอบครัวเดินต่อได้จริง`,
    `.`,
    `### ทำไมเรื่องนี้สำคัญ ###`,
    `เพราะถ้ารายได้หยุด แต่ภาระยังอยู่ ความกังวลจะไม่ใช่แค่เรื่องเงิน`,
    `มันจะกลายเป็นเรื่องของคนข้างหลัง บ้าน ลูก หนี้ ค่าใช้จ่าย และความพร้อมของทั้งบ้าน`,
    `.`,
    `### สิ่งที่ควรเช็กก่อนตัดสินใจ ###`,
    `1. ใครคือคนที่ต้องพึ่งรายได้ของเรา`,
    `2. ภาระที่ต้องรับผิดชอบมีอะไรบ้าง`,
    `3. งบต่อเดือนที่จ่ายไหวแบบไม่ฝืนคือเท่าไร`,
    `4. แผนที่เลือกอธิบายเงื่อนไขได้ชัดไหม`,
    `.`,
    ...
    `.`,
    `### ถ้าจะเริ่มจากตรงนี้ ###`,
    `เราชวนเริ่มจากการประเมินก่อน ไม่เร่งซื้อ ไม่เร่งปิด`,
    `ถ้าคุณอยากให้เราไล่ภาพรวมแบบทีละขั้น พิมพ์ “ประเมินแผน” มาได้เลย`,
    `เราจะช่วยสรุปให้ว่าอะไรควรเป็นเรื่องสำคัญก่อนหลัง`,
  ];
  const post = [
    ...opening,
    ".",
    `### สรุปสั้น ๆ ###`,
    `ถ้าคุณยังไม่มั่นใจว่าจะเริ่มจากตรงไหน ให้เริ่มจากชีวิตจริงก่อนเสมอ`,
    `เพราะการวางแผนที่ดีต้องตอบทั้งเรื่องคนที่ต้องดูแล รายได้ที่ต้องปกป้อง และงบที่จ่ายได้ต่อเนื่อง`,
    ".",
    `### ใช้ร่วมกับคอนเทนต์อื่นยังไง ###`,
    `- Facebook: ใช้โพสต์นี้เป็นตัวหลักที่เล่าเรื่องและสร้างความเชื่อใจ`,
    `- YouTube: ขยายเหตุผลให้ลึกขึ้นใน description และเนื้อหาคลิป`,
    `- TikTok / LINE VOOM: ตัดเอาแค่ประเด็นแรกกับ key message`,
    `- Blog SEO: แตกเป็นบทความยาวและ FAQ`,
    `- Email: ส่งต่อเป็น follow-up ที่ช่วยคนอ่านตัดสินใจต่อ`,
    ".",
    hashtags,
  ];
  return post.join("\n");
